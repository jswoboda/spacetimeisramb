...
In order to give a comparison a phantom was also created with no motion. This can be seen in Figure \ref{fig:phantomstat}. Images using the same integration time as in Figure \ref{fig:variable} for the stationary phantom can be seen in Figure \ref{fig:variablestat}. Another set of images using the longer integration time can be seen in Figure \ref{fig:blurredstat}. These images show that the blurring is on the same order between both integration times.
...
Lastly we show results of a simulation of with a full fitting. We again use a plasma enhancement moving through the field of view at 500 m/s but the electron and ion tempreture varies with time and altitude. The 
