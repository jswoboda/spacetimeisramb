...
%DIF ADD spacetimeisramb.tex       Thu Feb 19 15:43:28 2015
...
These new ESA based systems differentiate themselves from dish antennas in a fundamental way. Instead of dwelling in a single beam or scanning along a prescribed direction, an ESA can move to a different beam position within its field of view on a rapid, pulse by pulse basis. \DIFaddbegin \DIFadd{We can see this by how the different types of antennas integrate pulses using the beam positions seen in Figure \ref{fig:bp1}. Figure \ref{fig:dbsts} shows how a dish based antenna will move to different beam positions through time, represented by the z-axis in pulse repetition intervals (PRIs), by sweeping through the field of view in a continuous scan. The phased array will instead move from position to position in discrete steps as seen in Figure \ref{fig:phbsts}. Another observation from these diagrams is that the phased array antenna is able to collect data from different beams during overlapping time periods.
}

\begin{figure}
	\centering
	\includegraphics[width=3.5in]{beampositionssts}
	\caption{\DIFaddFL{Full spatial ambiguity function in Cartesian space for case with 4 beams with trailing edge of a 240$\mu$s pulse at 400km range.}}	
	\label{fig:bp1}
\end{figure}
...
\begin{figure}
	...
	\caption{\DIFaddFL{Space-time sampling of a dish based antenna.}}	
	\label{fig:dbsts}
\end{figure}
