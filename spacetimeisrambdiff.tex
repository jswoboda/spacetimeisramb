...
%DIF ADD spacetimeisramb.tex       Fri Feb 27 14:55:29 2015
...
These new ESA based systems differentiate themselves from dish antennas in a fundamental way. Instead of dwelling in a single beam or scanning along a prescribed direction, an ESA can move to a different beam position within its field of view on a rapid, pulse by pulse basis. \DIFaddbegin \DIFadd{We can see this by how the different types of antennas integrate pulses using the beam positions seen in Figure \ref{fig:bp1}. Figure \ref{fig:dbsts} shows a possible path for a dish based antenna to move to different beam positions through time, represented by the z-axis in pulse repetition intervals (PRIs). The dish is basically sweeping through the field of view in a continuous scan. The phased array will instead move from position to position in discrete steps as seen in Figure \ref{fig:phbsts}. Another observation from these diagrams is that the phased array antenna is able to collect data from different beams during overlapping time periods, which creates a lattice like pattern.  
}

\begin{figure}
	\centering
	\includegraphics[width=3.5in]{beampositionssts}
	\caption{\DIFaddFL{A 3x3 grid of beam positions to show sampling. }}	
	\label{fig:bp1}
\end{figure}
...
\noindent The range ambiguity function, $W(\tau_s,r_s,\tau,r)$, can be thought of as a smoothing operator along the range and lag dimensions of $R(\tau,r)$. \DIFaddbegin \DIFadd{This result is similar to what has been seen in \mbox{%DIFAUXCMD
\citet{nikoukar2008}
}%DIFAUXCMD
, \mbox{%DIFAUXCMD
...
, to ones that are determined using a theoretical model of the plasma ACF that have had the range ambiguity function applied to them. Our simulations produce plasma }\DIFaddend parameter scalar values of electron density, electron temperature, ion temperature, and ion composition. \DIFaddbegin \DIFadd{Lastly, to plot the three dimensional structures after fitting we use a natural neighbors interpolation \mbox{%DIFAUXCMD
\citep{Semeter2009738}
}%DIFAUXCMD
.
}\DIFaddend 
