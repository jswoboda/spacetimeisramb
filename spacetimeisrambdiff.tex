...
%DIF DEL spacetimeisramborig.tex   Wed Feb 11 14:53:44 2015
%DIF ADD spacetimeisramb.tex       Tue Mar  3 14:42:47 2015
...
Multiple ESA based ISR systems operate currently in the high latitude region where the ionosphere is highly variable in both space and time. Because of the highly dynamic nature of the ionosphere in this region, it is important to differentiate between measurement induced artifacts and the true behavior of the plasma. Often three dimensional ISR data produced by ESA \DIFdelbegin \DIFdel{techniques }\DIFdelend \DIFaddbegin \DIFadd{systems }\DIFaddend are fitted in a spherical coordinate space and then the parameters are interpolated to a Cartesian grid, \DIFdelbegin \DIFdel{introducing potential }\DIFdelend \DIFaddbegin \DIFadd{potentially introducing }\DIFaddend error and impacting the reconstructions of the plasma parameters.

To take advantage of the new flexibility inherent in ESA systems, we present a new way of analyzing ISR observations through use of the space-time ambiguity function. \DIFdelbegin \DIFdel{This concept is similar to the range ambiguity function that is used in traditional ISR for scanning antenna systems, but we have extended the concept to all spatial dimensions along with time as well. 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend The use of this new measurement ambiguity function \DIFdelbegin \DIFdel{allow }\DIFdelend \DIFaddbegin \DIFadd{allows }\DIFaddend us to pose the ISR observational problem in terms of a linear inverse problem whose goal is \DIFdelbegin \DIFdel{measurement }\DIFdelend \DIFaddbegin \DIFadd{the estimate }\DIFaddend of the time domain lags of the intrinsic plasma autocorrelation function \DIFaddbegin \DIFadd{used for parameter fitting}\DIFaddend . The framework allows us to explore the impact of non-uniformity in plasma parameters in both time and space. We discuss examples of possible artifacts in high latitude situations, and \DIFdelbegin \DIFdel{explore }\DIFdelend \DIFaddbegin \DIFadd{discuss possible }\DIFaddend ways of reducing them and improving the quality of data products from electronically steerable ISRs.
...
Incoherent scatter radar (ISR) is a powerful tool for exploring the ionosphere. These systems can give measurements of electron density $N_e$, ion temperature $T_i$, electron temperature $T_e$, ion velocity $V_i$ and other plasma parameters \citep{dougherty:farley1960, farleydougherty:ISR2, doughteryfarley:ISR3, hagfors1961}. These parameters are measured by fitting a nonlinear first-principles, physics based \DIFdelbegin \DIFdel{autocorrelation function (ACF) model to an estimated time autocorrelation or, alternatively, }\DIFdelend \DIFaddbegin \DIFadd{model of }\DIFaddend the power spectrum of the \DIFdelbegin \DIFdel{radar signal scattered off of }\DIFdelend \DIFaddbegin \DIFadd{signal scattered from the }\DIFaddend random electron density fluctuations \DIFaddbegin \DIFadd{to one that is estimated by the radar. Alternatively, this fitting can be done in the lag domain buy using the intrinsic autocorrelation function (ACF) of the plasma, which can be determined by taking an inverse Fourier transform of the power spectrum}\DIFaddend \citep{Lehtinen1996435}. 

This is an estimation of a second order statistic of an inherently random process from the scattering of electrons. In order to get an estimate of the ACF with reasonable statistical properties, an ensemble average must be performed by averaging \DIFdelbegin \DIFdel{pulses together }\DIFdelend \DIFaddbegin \DIFadd{power spectra or autocorrelation functions together from different pulses}\DIFaddend . With traditional dish antennas, ISR systems build  statistics in a limited number of ways. One method consists of pointing the radar beam in a specific direction and dwelling until enough pulses are integrated to get the desired statistics. Alternatively, the beam can be scanned through a field of view, collecting pulses while moving. These techniques use an implicit assumption about the uniformity of the plasma parameters within a volume defined by the pulse shape and solid angle beam properties while pulses are being integrated. This leads to an assumption of stationarity of the ACF within a temporal and spatial resolution cell of the radar. 
...
\DIFaddend Recently, electronically steerable array (ESA) technology has started to be leveraged by the ISR community. The Advanced Modular Incoherent Scatter Radar (AMISR) systems have already been deployed both at the Poker Flat Alaska (PFISR) and Resolute Bay Canada (RISR) geospace facilities. The European led EISCAT-3D project is currently being developed using phased array technology as well and will be capable of multistatic processing. These new systems are already being used in a number of different ways including creating volumetric reconstructions of plasma parameters \citep{Semeter2009738, Nicolls:2007ie, dahlgren2012di,Dahlgren:2012dq}. These reconstructions primarily consist of recasting ISR data into a Cartesian space through interpolation, after parameters have first been fit in a spherical coordinate system. \DIFaddbegin \DIFadd{Others have reconstructed full vector parameters using estimates of the ion velocity which can be determined using the Doppler shift of spectra \mbox{%DIFAUXCMD
\citep{butler:imagingfregiondrifts,RDS:RDS20195}
}%DIFAUXCMD
.
}\DIFaddend 

These new ESA based systems differentiate themselves from dish antennas in a fundamental way. Instead of dwelling in a single beam or scanning along a prescribed direction, an ESA can move to a different beam position within its field of view on a rapid, pulse by pulse basis. \DIFdelbegin \DIFdel{This }\DIFdelend \DIFaddbegin \DIFadd{We can see this by how the different types of antennas integrate pulses using the beam positions seen in Figure \ref{fig:bp1}. Figure \ref{fig:dbsts} shows a possible path for a dish based antenna to move to different beam positions through time, represented by the z-axis in pulse repetition intervals (PRIs). The dish is basically sweeping through the field of view in a continuous scan. The phased array will instead move from position to position in discrete steps as seen in Figure \ref{fig:phbsts}. Another observation from these diagrams is that the phased array antenna is able to collect data from different beams during overlapping time periods, which creates a lattice like pattern. This type of pulse-to-pulse beam position changes is very difficult to accomplish with dish antenna systems having significant pointing inertia. 
}

\begin{figure}
	\centering
	\includegraphics[width=3.5in]{beampositionssts}
	\caption{\DIFaddFL{A 3x3 grid of beam positions. }}	
	\label{fig:bp1}
\end{figure}
...
\begin{figure}
	...
	\caption{\DIFaddFL{Space-time sampling of a dish based antenna where the red x's mark the pulse in beam space and time. Beam positions from Figure \ref{fig:bp1} are shown below in blue at $z=0$.}}	
	\label{fig:dbsts}
\end{figure}
...
\begin{figure}
	...
	\caption{\DIFaddFL{Space-time sampling of a phased array based antenna where the red x's mark the pulse in beam space and time. Beam positions from Figure \ref{fig:bp1} are shown below in blue at $z=0$.}}	
	\label{fig:phbsts}
\end{figure}

\DIFadd{The change in the basic space-time sampling }\DIFaddend yields a new flexibility\DIFaddbegin \DIFadd{, in post processing, }\DIFaddend to statistically combine information from different beams using knowledge of the plasma velocity field, where this information is obtained either from external sources or from the Doppler shift of the ionospheric echoes themselves. This can help to relax the assumption of stationarity \DIFdelbegin \DIFdel{, since if the }\DIFdelend \DIFaddbegin \DIFadd{for plasmas that are evolving or changing their shape on time scales longer than the integration time. If the }\DIFaddend plasma moves into a different beam, returns from the same plasma can be integrated together with proper bookkeeping. \DIFdelbegin \DIFdel{Such a technique is very difficult to accomplish with dish antenna based systems having significant pointing inertia, and typically in these situations the returns from different plasmas }\DIFdelend \DIFaddbegin \DIFadd{This is contrary to the situation with dish antennas where returns from multiple plasma populations with different parameter sets }\DIFaddend are unavoidably and improperly averaged together.

In order to take advantage of new ESA flexibilities, this work puts forth the idea of the space-time ambiguity function. This concept extends the range ambiguity to all three spatial dimensions along with time. The goal of this paper is to develop the formalism for treating space-time ambiguity for electronically steerable ISRs, and in particular ISRs that are capable of sampling a given volume on a pulse-by-pulse basis\DIFdelbegin \DIFdel{.  }\DIFdelend \DIFaddbegin \DIFadd{, which has not been formalized. This paradigm can also be applied to other types ISR systems as well, but much of the utility of using this new formalism will likely be seen with ESA based systems. }\DIFaddend We will develop specific cases of the impact of the three-dimensional ambiguity on moving plasma using conditions characteristic of polar cap patches. A simulation of a polar cap patch using a full ISR simulator, which creates ISR data at the I/Q level, will be shown. Lastly we will \DIFdelbegin \DIFdel{explore a number of different }\DIFdelend \DIFaddbegin \DIFadd{quickly discuss }\DIFaddend strategies that could improve measurements from electronically steerable ISR systems.

%%%%%%%%%%%%%% Ambiguity Derivation%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Space-Time Ambiguity}

The space-time ambiguity can be thought of as a kernel to a combined volume and time integration operator. \DIFdelbegin \DIFdel{This }\DIFdelend \DIFaddbegin \DIFadd{In the coming derivations we will show this }\DIFaddend ambiguity can be represented as a kernel operator in a Fredholm integral equation:

\begin{equation}
\label{eqn:friedholm}
\rho(\tau_s ,\mathbf{r}_{s},t_s) = \int L(\tau_s, \mathbf{r}_{s},t_s,\tau,\mathbf{r},t) R(\tau,\mathbf{r},t) \DIFdelbegin \DIFdel{d\mathbf{r}d }\DIFdelend \DIFaddbegin \DIFadd{dVd }\DIFaddend t d\tau
\end{equation}

\noindent where, for ISR, $R(\tau,\mathbf{r},t) $ is the lag\DIFaddbegin \DIFadd{, $L(\tau_s, \mathbf{r}_{s},t_s,\tau,\mathbf{r},t) $ is a blurring kernel over time and space, }\DIFaddend $\tau$ of the autocorrelation function at time $t$, and position $\mathbf{r}$.

By using this formulation, many parallels between ISR and classic camera blurring problems can be made. In cameras, blurring can take place when an object moves over a space covered by one pixel while the shutter is open and the CCD is collecting photons. A diagram of this can be seen in Figure \ref{fig:ccd}. The same holds for the ISR measurement problem, except that the pixels are no longer square \DIFaddbegin \DIFadd{or continuous in Cartesian space }\DIFaddend and instead are determined by the beam shape and pulse pattern. This is shown in the diagrams in Figure \ref{fig:radarblur}.
...
Before we derive the full space-time ambiguity function\DIFdelbegin \DIFdel{$K(\mathbf{r}_{s},\mathbf{r})$ }\DIFdelend \DIFaddbegin \DIFadd{, $L(\tau_s,\mathbf{r}_s,t_s,\tau,\mathbf{r},t)$, }\DIFaddend we will start with defining our coordinate system.  Our three dimensional coordinate system is defined as $\mathbf{r}=[x,y,z]^T$. For this coordinate system, $\mathbf{r}=[0,0,0]^T$ at the location of the radar and thus $r=|\mathbf{r}|$, also known as the range variable. This allows for the use of polar coordinates $\mathbf{r} =  [r,\theta_,\phi]^T$ where $\theta$ \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{and $\phi$ are, respectively, }\DIFaddend the observer's elevation \DIFdelbegin \DIFdel{angle and $\phi$ is the azimuth angle}\DIFdelend \DIFaddbegin \DIFadd{and azimuth angles}\DIFaddend .

The radar samples this space at a set of discrete points which will be referred to as $\mathbf{r}_s = [x_s,y_s,z_s]^T$ along with the discretized range expression $r_s=|\mathbf{r}_s|$. The sampled space consists of a number of points, composed of range gates within a beam multiplied by the number of beams. These points can also be referred in polar coordinates $\mathbf{r}_s = [r_s,\theta_s,\phi_s]^T$, where $\theta_s$  \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{and $\phi_s$ are, respectively, }\DIFaddend the observationally sampled elevation \DIFdelbegin \DIFdel{angle and $\phi_s$ is the sampled azimuth angle}\DIFdelend \DIFaddbegin \DIFadd{and azimuth angles}\DIFaddend .

For notation purposes, we use two different sets of time commonly known in \DIFaddbegin \DIFadd{the hard-target }\DIFaddend radar literature as fast-time, $n$ and slow-time, $t$ \citep{richards:fundamentalsigproc}. Fast-time is used to describe processes with correlation time less \DIFdelbegin \DIFdel{then one pulse repetition interval (PRI)}\DIFdelend \DIFaddbegin \DIFadd{than one PRI}\DIFaddend . Slow-time will be used for processes that decorrelate in time on the order of, or longer than, the system's PRI. In order to form estimates of ACFs with desired statistical properties, it is assumed that the plasma parameters parameters will change on the order of many tens to hundreds of PRIs in their stationary reference frame (i.e. remain wide sense stationary for this time). Generally\DIFaddbegin \DIFadd{, }\DIFaddend for incoherent scatter applications in the E-region of the ionosphere (\DIFdelbegin \DIFdel{~}\DIFdelend \DIFaddbegin \DIFadd{$\approx$}\DIFaddend 100 km altitude) and above, \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{with systems that have a center frequency in the UHF band, the }\DIFaddend decorrelation time is less than a PRI, and thus ACFs must be formed over fast-time.

The terms $n$ and $t$ represent continuous variables, while $n_s$ and $t_s$ will be the fast time and slow time parameters sampled by the radar. The sampling rate of $n_s$ is set by the rate at which the system's A/D converters are run. The sampling of $t_s$ can, at the highest rate, be the PRI. At its lowest rate, it can be sampled once in a non-coherent processing interval (NCPI), or equivalently in a period of time it takes the radar to average the desired number of pulses \DIFaddbegin \DIFadd{for each beam}\DIFaddend . 

\subsection{Derivation}

The basic physical mechanism behind ISR produces measurable radar scatter from electron density fluctuations in the ionosphere, $n_e(\mathbf{r},n)$\DIFaddbegin \DIFadd{, at a specific wavenumber $\mathbf{k}$}\DIFaddend . These fluctuations scatter radio waves which can be observed by the receiver system of the radar \citep{dougherty:farley1960}. The emitted radar signal at the transmitter has a pulse shape $s(n)$ modulated at a central frequency creating a scattering wave number $\mathbf{k}$. Using the Born approximation, the signal received at time $n$, $x(n)$, can be represented as the following
...
\noindent We can make a simplifying assumption at this point that the space-time autocorrelation function of $n_e(\mathbf{r},t)$, $\langle n_e(\mathbf{r},n)n_e(\mathbf{r}',n+\tau)\rangle$, will go to zero as the magnitude of $\mathbf{y} \equiv \mathbf{r}'-\mathbf{r}$ increases beyond the debye length \citep{farley1969}. Thus, the rate which the spatial autocorrelation goes to zero will be such that $\tau\gg \frac{2||\mathbf{y}||}{c}$, allowing us to set $r= r'$ \DIFaddbegin \DIFadd{inside the arguments of $s$ and $h$}\DIFaddend . This allows Equation \ref{ex:acf1} to be rewritten as 

 \begin{multline}
 \label{ex:acf2}
 \langle x(n_s)x^*(n_s+\tau)\rangle = \int s\left(n-\frac{2r}{c}\right)s^*\left(n+\tau -\frac{2r}{c}\right) h(n_s-n)h^*(n_s+\tau_s-n-\tau) \\\left[\int \exp\left[-2j \mathbf{k}\cdot \mathbf{y}\right] \langle n_e(\mathbf{r},n)n^*_e(\mathbf{y}+\mathbf{r},n+\tau)\rangle d\mathbf{y} \right]\DIFaddbegin \DIFadd{drdn }\DIFaddend d\DIFdelbegin \DIFdel{\mathbf{r}dn d}\DIFdelend \tau.
 \end{multline}
...
\noindent The range ambiguity function, $W(\tau_s,r_s,\tau,r)$, can be thought of as a smoothing operator along the range and lag dimensions of $R(\tau,r)$. \DIFaddbegin \DIFadd{This result is the same as what has been seen in \mbox{%DIFAUXCMD
\citet{nikoukar2008}
}%DIFAUXCMD
, \mbox{%DIFAUXCMD
\citet{Woodman:1991is}
}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\cite{hysell2008}
}%DIFAUXCMD
...
The spatial ambiguity is a separable function made up of the components of $W(\tau_s,\tau,r_s,r)$ and $F(\theta_s,\phi_s,\theta,\phi)$. These two functions can be combined by multiplying the two, creating the spatial ambiguity function  $K(\tau_s,\mathbf{r}_s,\tau,\mathbf{r})$\DIFdelbegin \DIFdel{, and then doing a volume integration}\DIFdelend . This yields an expression for a single statistical realization of the ACF of the incoherent scatter random process, which will be referred to as $\rho(\tau_s,\mathbf{r}_s)$:


 \begin{align}
  \label{eqn:volume}
\rho(\tau_s,\mathbf{r}_s) &= \int F(\theta_s,\phi_s,\theta,\phi)W(\tau_s,r_s,\tau,r) R(\tau,\mathbf{r}) dV \DIFaddbegin \DIFadd{d\tau }\DIFaddend ,\\
	&= \int K(\tau_s,\mathbf{r}_s,\tau,\mathbf{r}) R(\tau,\mathbf{r})  \DIFdelbegin \DIFdel{dV}\DIFdelend \DIFaddbegin \DIFadd{dVd\tau}\DIFaddend .
\end{align}
...
\begin{figure}
	...
	\caption{Full spatial ambiguity function in Cartesian space for case with 4 beams with trailing edge of a 240$\mu$s pulse at 400km range. \DIFaddbeginFL \DIFaddFL{The surface represents the half power point of the ambiguity function.}\DIFaddendFL }	
	\label{fig:amb4}
\end{figure}

As mentioned above, this one pulse ACF estimate represents a single sample of a random process. In order to create a usable estimate, multiple samples of this ACF need to be averaged together to reduce the variance to sufficient levels in order to fit the estimate to a theoretical ACF that is a direct function of plasma parameter values. To show the impact of this averaging in creating the estimate of the ACF, we will add slow-time dependence to the expression for the medium ACF, which now becomes $R(\tau,\mathbf{r},t)$, and will also add another separable function $G(t_s,t)$ to the kernel. This function $G(t_s,t)$ can be thought of as a sampling and blurring kernel for the ACF if the plasma parameters change within an NCPI. Since the amount of time that the radar pulse is illuminating the plasma in a point of space is very short compared to \DIFdelbegin \DIFdel{IPP}\DIFdelend \DIFaddbegin \DIFadd{the PRI}\DIFaddend , $G(t_s,t)$ can take the form of a summation of Dirac delta functions 

\begin{equation}
\label{eqn:Gexp}
G(t_s,t) = \displaystyle \sum_{j=0}^{J-1}\alpha_j \delta(t-t_s-jT\DIFdelbegin \DIFdel{_{PRI}}\DIFdelend \DIFaddbegin \DIFadd{_{REV}}\DIFaddend ),
\end{equation}

\noindent where $J$ is the number of pulses used over a NCPI, \DIFdelbegin \DIFdel{$T_{PRI}$ is the PRI time period }\DIFdelend \DIFaddbegin \DIFadd{$T_{REV}$ is the amount of time it takes the radar to revisit the specific beam }\DIFaddend and $\alpha_j$ is the weights that the radar assigns to the pulses. \DIFaddbegin \DIFadd{For systems using pulse-to-pulse steering one strategy can be to revisit each beam sequentially, in this case making $T_{REV}=N_{beam}T_{PRI}$, where $N_{beam}$ is the number of beams  $T_{PRI}$ is the PRI time period . }\DIFaddend The weights are generally set to $1/J$ to simply average the pulses. With Equation \ref{eqn:Gexp} incorporated into the overall ambiguity we obtain the full integral equation,

\begin{equation}
\label{eqn:sptamb}
	\rho(\tau_s,\mathbf{r}_s,t_s) =\int L(\tau_s,\mathbf{r}_s,t_s,\tau,\mathbf{r},t)R(\tau,\mathbf{r},t)\DIFdelbegin \DIFdel{dVdt}\DIFdelend \DIFaddbegin \DIFadd{dVdtd\tau}\DIFaddend .
\end{equation}
...
\begin{equation}
\label{eqn:L2}
\rho(\tau_s,\mathbf{r}_s,t_s) = \int K(\tau_s,\mathbf{r}_s,\tau,\mathbf{r}) \left[(1/J)\int_{t_s}^{t_s+T} \displaystyle \sum_{j=0}^{J-1} \delta(t-t_s-jT\DIFdelbegin \DIFdel{_{PRI}}\DIFdelend \DIFaddbegin \DIFadd{_{REV}}\DIFaddend )R(\tau,\mathbf{r},t) dt\right] \DIFdelbegin \DIFdel{dV}\DIFdelend \DIFaddbegin \DIFadd{dVd\tau}\DIFaddend .
\end{equation}

Of specific interest in this study are instances in the high latitude ionosphere where embedded plasma structures are moving due to electric field drivers applied by the magnetosphere. In this case, it will be assumed that the plasma is a rigid object and will not deform with respect to $\mathbf{r}$ over time period $[t_0,t_0+T]$ where \DIFdelbegin \DIFdel{$T=JT_{PRI}$ }\DIFdelend \DIFaddbegin \DIFadd{$T=JT_{REV}$ }\DIFaddend is the time for one NCPI. Also, it will be assumed that the plasma parcel moves with a constant velocity $\mathbf{v}$. Thus $R(\tau,\mathbf{r},t)\Rightarrow R(\tau,\mathbf{r}+\mathbf{v}t)$. The assumption of rigidity \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{can be }\DIFaddend valid over the time period of the NCPI, on the order of a few minutes, while the plasma moves through the field of view of the radar \DIFaddbegin \DIFadd{for some cases}\DIFaddend . For example, in the high latitude ionosphere, large scale features in structures such as patches decay on the order of hours \citep{Tsunoda:1988ul}. This assumption is useful because it allows our framework to analyze impacts of these plasma variations on the parameter resolution of ISR systems. With these assumptions, Equation \ref{eqn:L2} becomes,

\begin{equation}
\label{eqn:L3}
\rho(\tau_s,\mathbf{r}_s,t_s) =(1/J) \int \int_{t_s}^{t_s+T} \displaystyle \sum_{j=0}^{J-1}\delta(t-t_s-jT\DIFdelbegin \DIFdel{_{PRI}}\DIFdelend \DIFaddbegin \DIFadd{_{REV}}\DIFaddend ) K(\tau_s,\mathbf{r}_s,\tau,\mathbf{r})R(\tau,\mathbf{r}+\mathbf{v}t)\DIFdelbegin \DIFdel{dtdV}\DIFdelend \DIFaddbegin \DIFadd{dtdVd\tau}\DIFaddend \end{equation}
...
\begin{equation}
\label{eqn:L5}
\rho(\tau_s,\mathbf{r}_s,t_s)= (1/J)\int \left[ \;\;  \displaystyle \sum_{j=0}^{J-1} K(\tau_s,\mathbf{r}_s,\tau,\mathbf{r}'-\mathbf{v}(t_s+jT\DIFdelbegin \DIFdel{_{PRI}}\DIFdelend \DIFaddbegin \DIFadd{_{REV}}\DIFaddend )) \;\; \right]R(\tau,\mathbf{r}')\DIFdelbegin \DIFdel{dV}\DIFdelend \DIFaddbegin \DIFadd{dVd\tau}\DIFaddend .
\end{equation}
...
\begin{equation}
\label{eqn:L6}
\rho(\tau_s,\mathbf{r}_s,t_s)= \int A(\tau_s,\mathbf{r}_s,t_s,\tau,\mathbf{r}') R(\tau,\mathbf{r}')\DIFdelbegin \DIFdel{dV}\DIFdelend \DIFaddbegin \DIFadd{dVd\tau}\DIFaddend .
\end{equation}
...
\begin{figure}[!t]
	...
	\caption{Same spatial ambiguity as in Figure \ref{fig:amb4} but now with 500 m/s velocity in $y$ direction in plasma frame of reference. \DIFaddbeginFL \DIFaddFL{The surface represents the half power point of the ambiguity function}\DIFaddendFL }
	\label{fig:ambtime}
\end{figure}

The operator $A$ can be determined through knowledge of the radar system's beam pattern along with the experiment's pulse pattern, integration time and inherent velocity of the plasma. This velocity $\mathbf{v}$ \DIFdelbegin \DIFdel{can }\DIFdelend \DIFaddbegin \DIFadd{could }\DIFaddend be separately estimated by taking measurements of the Doppler shift \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{by }\DIFaddend using a methodology \DIFaddbegin \DIFadd{like that }\DIFaddend seen in \citet{butler:imagingfregiondrifts}. \DIFdelbegin \DIFdel{Once the operator has been determined, standard processing techniques can be used as if the plasma is not moving, under the previous assumptions. 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{With this strategy the operator is now acting purely as a spatial blurring function instead of a full space-time function. In the end, reducing dimensionality of the problem can make it easier to solve the inverse problem and allow one to improve the measurement. 
...
\caption{ISR processing chain and data products\DIFaddbeginFL \DIFaddFL{, in diamonds, }\DIFaddendFL from each step.}
\label{fig:chain}
\end{figure}

The sampled I/Q voltages can be represented as $x(n_s) \in\mathbb{C}^N$ where $N$ is the number of samples in \DIFdelbegin \DIFdel{an inter pulse period}\DIFdelend \DIFaddbegin \DIFadd{a PRI}\DIFaddend .  At this point, the first step in estimating the autocorrelation function is taken.  For each range gate $m\in 0,1,...M-1$ an autocorrelation is estimated for each lag of $l \in 0,1...,L-1$.  This operation of forming the ACF estimates repeats for each pulse, $j\in 0,1,...J-1$, and is then summed over the $J$ pulses. The entire operation to form the initial estimate of $\hat{R}(m,l)$ is the following,
...
The final estimate of the autocorrelation function after the noise subtraction and summation rule will be represented by $\hat{R}_f(m,l)$. At this point, a summation rule is applied and the data is sent off to be fit. The final parameters are derived through \DIFaddbegin \DIFadd{fitting these estimated ACFs, with }\DIFaddend a standard Levenberg-Marquardt non-linear least-squares \DIFdelbegin \DIFdel{fitting \mbox{%DIFAUXCMD
\citep{levenberg1944}
}%DIFAUXCMD
producing plasma }\DIFdelend \DIFaddbegin \DIFadd{algorithm \mbox{%DIFAUXCMD
\citep{levenberg1944}
}%DIFAUXCMD
, to ones that are determined using a theoretical model of the plasma ACF that have had the range ambiguity function applied to them. Our simulations produce plasma }\DIFaddend parameter scalar values of electron density, electron temperature, \DIFdelbegin \DIFdel{ion temperature, and ion composition}\DIFdelend \DIFaddbegin \DIFadd{and ion temperature. Lastly, to plot the three dimensional structures after fitting we use a natural neighbors interpolation as seen in \mbox{%DIFAUXCMD
\citet{Semeter2009738}
}%DIFAUXCMD
}\DIFaddend .
...
\begin{figure}
	...
	\caption{The 121 \DIFdelbeginFL \DIFdelFL{Radar }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{radar }\DIFaddendFL beam \DIFdelbeginFL \DIFdelFL{pattern }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{positions }\DIFaddendFL used in the simulations.}	
...
\begin{figure}
	...
	\caption{Reconstructions of $N_e$ using 10 \DIFdelbeginFL \DIFdelFL{Pulses }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{pulses }\DIFaddendFL at three different times from the input shown in Figure \ref{fig:phantom}.}	
	\label{fig:variable}
\end{figure}
...
\begin{figure}
	...
	\caption{Reconstructions of $N_e$ using 200 \DIFdelbeginFL \DIFdelFL{Pulses }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{pulses }\DIFaddendFL at three different times from the input shown in Figure \ref{fig:phantom}.}	
	\label{fig:blurred}
\end{figure}

In order to give a comparison based on integration time, a phantom was also created with no motion. This can be seen in the first pane of Figure \ref{fig:statreconstruct}. An image using the same integration time as in Figure \ref{fig:variable} for the stationary phantom is shown as the center pane in Figure \ref{fig:statreconstruct}. Another image using the longer integration time\DIFaddbegin \DIFadd{, as in Figure \ref{fig:blurred} }\DIFaddend can be seen in the right pane of Figure \ref{fig:statreconstruct}. These images show that the blurring is on the same order between both integration times \DIFaddbegin \DIFadd{contrasting with the figures where the plasma is moving}\DIFaddend .
...
We again use a plasma enhancement moving through the field of view at 500 m/s, but the electron and ion temperature varies with time and altitude. The  background ion and electron temperature vs. height can be seen in Figure \ref{fig:allparamsphantom}. As the electron density enhancement feature travels through the field of view, the ion and electron temperature \DIFdelbegin \DIFdel{ratio }\DIFdelend is set to drop by the same ratio that the electron density is enhanced. This \DIFdelbegin \DIFdel{is done to keep the variance the same at each point in space for a given number of pulses integrated}\DIFdelend \DIFaddbegin \DIFadd{was done to add a corresponding variation in the temperatures along with the density}\DIFaddend . 

\begin{figure}
	\centering
	\includegraphics[width=4in]{tempvsheight}
	\caption{Background ion \& electron temperature verses height used for the simulation \DIFdelbeginFL \DIFdelFL{. }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{in case 2. }\DIFaddendFL }	
	\label{fig:allparamsphantom}
\end{figure}

The phantoms for each parameter at approximately 402 seconds can be seen in Figure \ref{fig:allparamsphantom}. The reconstruction of this field \DIFaddbegin \DIFadd{using a 3 minute integration time, 200 pulses, centered }\DIFaddend at the same time can be seen in Figure \ref{fig:allparamsblurreddata}. Note that the reconstruction does not seem to show the electron density enhancement, even in a blurred form. 
...
\noindent Since the second array can be though of a shifted version of the first in the $x$ \DIFdelbegin \DIFdel{direction }\DIFdelend \DIFaddbegin \DIFadd{and $y$ directions }\DIFaddend we get the following
...
\DIFaddend E_2(\theta,\phi)  \DIFaddbegin &\DIFaddend =  \exp\left[j(\psi_y/2 + \psi_x)\right] E_1(\theta,\phi)  \DIFaddbegin \\&\DIFaddend = \DIFaddbegin \DIFadd{\exp}\left[\DIFadd{j(\psi_y/2 + \psi_x)}\right]  \DIFaddend \displaystyle \sum_{m=1}^{M/2}\sum_{n=1}^{N}  \exp\left[-j2\left(m-1\right) \psi_x -j\left(n-1\right) \psi_y\right].
